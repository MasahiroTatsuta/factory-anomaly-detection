# -*- coding: utf-8 -*-
"""mimii_transformer_final

Automatically generated by Colab.
完全対照実験仕様：全ID混在 ＋ クラスウェイト7.7倍 ＋ 5エポック集中訓練
"""
...
class UnifiedAudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 後段の処理を高速化するためDataset側で224x224にリサイズして返却
...
# ==============================================================================
# --- 5. W&B管理の元、提案手法（ViT-Tiny）を5エポック集中訓練 ---
# ==============================================================================
import wandb
import torch.optim as optim
from tqdm import tqdm

wandb.login(key="YOUR_WANDB_API_KEY") # 自身のAPIキーを入力してください

wandb.init(
    project="factory-anomaly-detection",
    name="vit-tiny-final-generalized",
    config={
        "learning_rate": 1e-4,
        "architecture": "ViT-Tiny",
        "dataset": "MIMII Valve Unified",
        "epochs": 5,
        "batch_size": 64
    }
)
...
for epoch in range(epochs):
    ...
    for features, labels in tqdm(train_loader, desc=f"ViT Epoch {epoch+1}/{epochs}"):
        features, labels = features.to(device), labels.to(device)

        vit_optimizer.zero_grad()
        outputs = vit_model(features)
        loss = criterion(outputs, labels) # 体重付きLoss
...
# 本番API（app.py）へシームレスに移植するため、接頭辞なしの純粋な重みとして保存
...
for epoch in range(5): # 全く同じ5エポック
    cnn_model.train()
    running_loss = 0.0
    for inputs, labels in tqdm(train_loader, desc=f"CNN Epoch {epoch+1}/{5}"):
        ...
        loss = criterion(cnn_model(inputs), labels) # 全く同じ体重Loss
        loss.backward()
        cnn_optimizer.step()
        running_loss += loss.item()
...
# 物理ファイル名衝突を避けるため、パスをパースしてマシーンIDを付与してコピー
for f_path, lbl in test_files_flat:
    # パスから 'id_XX' と 'filename.wav' を抽出
    parts = f_path.split('/')
    m_id = parts[-3] # 'id_0X'
    f_name = parts[-1] # 'XXXX.wav'
